...
/**
 * Generate the Playground-flavored Range shim. The shim translates
 * `RANGE_*` env vars into Playground's training-script flags so
 * sweep params (RANGE_SEED) and Range-side concerns (RANGE_RUN_DIR,
 * RANGE_CHECKPOINT) flow correctly without modifying Playground's
 * code. Edit the file in the user's repo to customize.
 */
function buildPlaygroundShim(d: PlaygroundDiscovery): string {
  ...
  return `"""Range shim for MuJoCo Playground — auto-generated by Range.

Range launches scenarios by spawning the \`args\` from \`range.yaml\` as a
child process and injecting \`RANGE_*\` env vars (sweep params, run
output dir, checkpoint paths, …). Playground's training scripts use
\`absl.flags\`; this shim is the glue between the two.

Edit this file freely — it lives in your repo, not Range's. Each
mode is one branch:

  uv run python tools/range_shim.py run    # train (or eval if RANGE_CHECKPOINT)
  uv run python tools/range_shim.py smoke  # tiny smoke check
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: range_shim.py <run|smoke> [extra args...]", file=sys.stderr)
        return 2

    mode = sys.argv[1]
    extra = sys.argv[2:]

    env_name = env("RANGE_ENV_NAME") or env("RANGE_SCENARIO_NAME") or "CartpoleBalance"
    seed = env("RANGE_SEED", "42")
    run_dir = env("RANGE_RUN_DIR")
    num_timesteps = env("RANGE_NUM_TIMESTEPS")
    checkpoint = env("RANGE_CHECKPOINT")

    cmd: list[str] = ["python", TRAIN_SCRIPT, f"--env_name={env_name}", f"--seed={int(seed)}"]

    if mode == "smoke":
        # Override to a tiny training budget regardless of caller.
        cmd.append("--num_timesteps=100000")
    elif num_timesteps:
        cmd.append(f"--num_timesteps={num_timesteps}")

    if run_dir:
        # Most Playground training scripts honor a --logdir or
        # similar flag; if yours doesn't, set the working dir
        # elsewhere or pipe artifacts post-hoc.
        cmd.append(f"--logdir={run_dir}")

    if checkpoint:
        # From Range's /eval slash. Playground's PPO trainer takes
        # a restore-checkpoint flag — name may differ; tweak here.
        cmd.append(f"--restore_checkpoint_path={checkpoint}")

    cmd.extend(extra)
    print(f"shim → {' '.join(cmd)}", flush=True)
...
async function buildPlaygroundYaml(
  ...
  for (const s of scenarios) {
    ...
    lines.push(`    description: ${s.envName} via tools/range_shim.py`);
    lines.push(
      `    args: [${pyArgs}, "tools/range_shim.py", "run"]`,
    );
    lines.push("    env:");
    lines.push(`      RANGE_ENV_NAME: "${s.envName}"`);
  }
...
async function detectPlayground(
  ...
  return {
    ...
    shim: {
      path: "tools/range_shim.py",
      content: buildPlaygroundShim(d),
      description:
        "Bridges Range's RANGE_* env vars to Playground's absl flags so sweeps + /eval work without code changes upstream.",
    },
    summary,
    notes,
  };
...
/**
 * Write the proposed YAML to `<repoPath>/range.yaml`, plus an
 * optional shim file. Refuses if either path already exists — we
 * never overwrite a user's profile or scripts. Best-effort: if the
 * yaml writes but the shim fails (e.g., a stale `tools/range_shim.py`
 * is in the way), the yaml stays and the error surfaces.
 */
export async function writeScaffold(
  repoPath: string,
  yamlText: string,
  shim?: { path: string; content: string },
): Promise<{
  written: boolean;
  paths: string[];
  error?: string;
}> {
  const yamlPath = join(repoPath, RANGE_YAML);
  if (await exists(yamlPath)) {
    return {
      written: false,
      paths: [],
      error: `${basename(yamlPath)} already exists — delete it before accepting a new scaffold.`,
    };
  }

  // Resolve + validate the shim path BEFORE writing the yaml, so we
  // don't end up half-applied if the shim path is bogus.
  let shimPath: string | null = null;
  if (shim) {
    // Defend against `..` traversal — the shim path must stay inside
    // the repo. We don't trust the client-supplied path blindly.
    const rel = shim.path.replace(/^\/+/, "");
    if (rel.includes("..")) {
      return {
        written: false,
        paths: [],
        error: `shim path contains '..' (would escape repo): ${shim.path}`,
      };
    }
    shimPath = join(repoPath, rel);
    if (await exists(shimPath)) {
      return {
        written: false,
        paths: [],
        error: `${shim.path} already exists — delete it before accepting a new scaffold.`,
      };
    }
  }

  await writeFile(yamlPath, yamlText, "utf8");
  const written: string[] = [yamlPath];

  if (shim && shimPath) {
    try {
      await mkdir(dirname(shimPath), { recursive: true });
      await writeFile(shimPath, shim.content, "utf8");
      written.push(shimPath);
    } catch (err) {
      return {
        written: true, // yaml landed
        paths: written,
        error: `shim write failed: ${String(err instanceof Error ? err.message : err)}`,
      };
    }
  }
