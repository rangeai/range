...
function buildPlaygroundYaml(d: PlaygroundDiscovery): string {
  ...
  lines.push("commands:");
  lines.push("  test:");
  lines.push("    args: [\"pytest\", \"-q\", \"mujoco_playground\"]");
  lines.push("    description: Run Playground unit tests");
  lines.push("  smoke:");
  lines.push(
    `    args: [\"python\", \"${primaryTrain}\", \"--env_name=CartpoleBalance\", \"--num_timesteps=100000\"]`,
  );
  ...
  for (const s of scenarios) {
    ...
    lines.push(
      `    args: [\"python\", \"${primaryTrain}\", \"--env_name=${s.envName}\"]`,
    );
    lines.push("    env:");
...
async function detectPlayground(
  ...
  return {
    ...
    yamlText: buildPlaygroundYaml(d),
    summary,
    notes,
  };
}
